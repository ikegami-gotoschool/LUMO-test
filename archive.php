      ...
      <section class="pageFaq_content pageColumn_content">
        <div class="inner _wide">
          <div class="pageColumn_box">
            <h2 class="pageFaq_heading heading _side _green nowrap pageColumn_heading">コラム一覧</h2>
                  ...
                  <?php while ($the_query->have_posts()) : $the_query->the_post(); ?>
                    <li class="allpost__li">
                      <a class="allpost__li__a" href="<?php the_permalink(); ?>">
                        ...
                        <?php
                        $categories = get_the_category(); // 投稿に紐付いたカテゴリーを取得

                        if (! empty($categories)) {
                          echo '<ul class="category-list">'; // ulタグで囲む
                          foreach ($categories as $category) {
                            echo '<li class="category-name">' . esc_html($category->name) . '</li>'; // liタグでカテゴリー名を囲む
                          }
                          echo '</ul>'; // ulタグの閉じ
                        }
                        ?>
